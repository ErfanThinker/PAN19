                ...
                for ind, aspect in enumerate(aspects):
                    try:
                        ...
                        if rsplit.startswith('p') or rsplit.startswith('u'):
                            ...
                            expCount += 1
                        aspectTerms.update({fileName + '_' + str(id) + '_' + str(ind): [aspectTerm,
                                                                                        1 if '+' in aspect.split('[')[
                                                                                            1] else -1,
                                                                                        rsplit.startswith(
                                                                                            'p') or rsplit.startswith(
                                                                                            'u')]})
                    except IndexError:
                        print(fileName)
                        print(sentence)
    ...
    for atype in e0.findall('sentence'):
        ...
        if (atype.find('aspectTerms')):
            ...
            for aspectTerm in aspectTermsElement.findall('aspectTerm'):
                aspectTerms.append(
                    [re.sub(regex, ' ', aspectTerm.get('term').lower()), aspectLabels[aspectTerm.get('polarity')],
                     aspectTerm.get('from'), aspectTerm.get('to')])
    ...
    try:
        with open('./Lexicons/MPQA.tff', 'r', encoding='ISO-8859-1') as txtFile:
            for index, line in enumerate(txtFile):
                ...
                word, score = splitedLine[2][6:], -1 if splitedLine[5] == 'priorpolarity=negative' else 1 if \
                splitedLine[5] == 'priorpolarity=positive' else 0
...
def writeFeatures(fileName, allTrainTests, predict, testComments):
    ...
    with open(fileName + '_test_features_' + '.csv', 'w', encoding='utf-8') as csvfile:
        spamwriter = csv.writer(csvfile, quoting=csv.QUOTE_MINIMAL)
        headers = ['id', 'aspectTerm', 'sentence', 'aspectPolarity', 'prediction'] + ['f' + str(i) for i, v in
                                                                                      enumerate(allTrainTests[0][0])]
